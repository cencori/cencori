...
async function handleAutoFix(
    ...
    try {
        ...
        // Separate actual fixes from failed ones (where AI couldn't generate a fix)
        ...
        if (actualFixes.length === 0) {
            ...
            // Save all failed fixes for manual review
        ...
        const acceptedFixes: typeof actualFixes = [];
        const skippedFixes: typeof actualFixes = [];
        let applyAll = false;
        let skipRest = false;
