...
export async function GET(
    ...
    try {
        ...
        const { groupBy } = getTimelineConfig(timeRange, now);
        ...
        if (error) {
            console.error('[Gateway Timeline API] Error fetching gateway logs:', error);
            return NextResponse.json(
                { error: 'Failed to fetch gateway timeline' },
                { status: 500 }
            );
        }

        const logRows = (logs as GatewayLogRecord[] | null) || [];
        if (logRows.length === 0) {
        ...
        console.error('[Gateway Timeline API] Unexpected error:', unexpectedError);
        return NextResponse.json(
