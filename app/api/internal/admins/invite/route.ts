...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        try {
            ...
            console.warn('[Admins] Email provider not configured. Skipping invite email.');
            return NextResponse.json({
                success: true,
                inviteLink,
                message: `Invite created for ${email}! Email skipped (provider not configured).`
            });
        }

        if (!ADMIN_INVITE_FROM_EMAIL) {
            console.warn('[Admins] Admin invite from email not configured. Skipping invite email.');
        } else {
