    ...
    <div className="w-full max-w-5xl mx-auto px-6 py-8 space-y-6">
      ...
      {agents && agents.length > 0 ? (
          ...
          {agents.map((agent) => (
            <div key={agent.id} className="group relative rounded-xl border border-border/40 bg-card p-5 hover:border-border/80 transition-all">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-primary/5 flex items-center justify-center text-primary">
                    {agent.blueprint === 'openclaw' ? <OpenClawLogo className="w-5 h-5" /> : agent.blueprint === 'autogpt' ? <AutoGPTLogo className="w-5 h-5" /> : agent.blueprint === 'n8n' ? <N8nLogo className="w-5 h-5" /> : agent.blueprint === 'crewai' ? <CrewAILogo className="w-5 h-5" /> : agent.blueprint === 'python-interpreter' ? <PythonLogo className="w-5 h-5" /> : agent.blueprint === 'custom' ? <CustomAgentLogo className="w-5 h-5" /> : <SparklesIcon className="w-5 h-5" />}
                  </div>
                  <div>
                    <h3 className="text-sm font-medium leading-none mb-1">{agent.name}</h3>
                    <span className={`inline-flex items-center gap-1.5 text-[10px] px-2 py-0.5 rounded-full border ${agent.is_active ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-600' : 'border-muted-foreground/20 bg-muted/10 text-muted-foreground'}`}>                        <span className={`w-1 h-1 rounded-full ${agent.is_active ? 'bg-emerald-500' : 'bg-muted-foreground'}`} />
              ...
              <Button variant="outline" size="sm" className="w-full h-8 text-xs" asChild>
                <Link href={`/dashboard/organizations/${orgSlug}/projects/${projectSlug}/agents/${agent.id}`}>                  Manage Agent
                </Link>
              </Button>
            </div>
          ))}
        ...
        <div className="text-center py-20 rounded-xl border border-dashed border-border/40 bg-muted/5">
          ...
          <Button size="sm" asChild>
            <Link href={`/dashboard/organizations/${orgSlug}/projects/${projectSlug}/agents/new`}>              Deploy Agent
            </Link>
          </Button>
        </div>
      )}
