...
import { ArrowRight, ChevronRight, Building2, Lock, FileSearch, ShieldAlert } from "lucide-react";
        ...
        <div className="min-h-screen bg-background text-foreground">
            ...
            <main>
                <section className="relative flex flex-col items-center justify-center overflow-hidden bg-background pt-32 pb-20">
                    <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-500/10 via-background to-background pointer-events-none" />
                    ...
                    <div className="container relative z-10 px-4 md:px-6 flex flex-col items-center text-center">
                        <div className="mb-8 animate-appear">
                            <span className="inline-flex items-center rounded-full border border-foreground/10 bg-foreground/5 px-3 py-1 text-sm font-medium text-foreground/80">
                                <span className="flex h-2 w-2 rounded-full bg-slate-500 mr-2 animate-pulse" />
                                <span className="mr-2">Enterprise Ready</span>
                            </span>
                        </div>
                        ...
                        <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tighter mb-6 max-w-4xl animate-appear [animation-delay:100ms] text-transparent bg-clip-text bg-gradient-to-b from-foreground to-foreground/50">
                            Scale AI with <span className="italic">confidence</span>
                        </h1>

                        <p className="text-lg md:text-xl text-muted-foreground max-w-xl mb-10 animate-appear [animation-delay:200ms] leading-relaxed">
                            Complete governance, observability, and security for enterprise AI adoption. ISO 27001 and SOC 2 Type II compliant.
                        </p>
                    ...
                    <div className="container mx-auto px-4 md:px-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
                            <div className="p-6 border border-border/30 rounded-lg">
                                <ShieldAlert className="h-6 w-6 text-red-500 mb-4" />
                                <h3 className="font-semibold text-lg mb-2">Threat Prevention</h3>
                                <p className="text-sm text-muted-foreground">
                                    Advanced protection against prompt injection, jailbreaks, and data exfiltration. Custom rule engine for organization-specific policies.
                                </p>
                            </div>
                            <div className="p-6 border border-border/30 rounded-lg">
                                <FileSearch className="h-6 w-6 text-blue-500 mb-4" />
                                <h3 className="font-semibold text-lg mb-2">Audit & Governance</h3>
                                <p className="text-sm text-muted-foreground">
                                    Immutable audit logs for every AI interaction. Comprehensive visibility into usage, costs, and policy violations.
                                </p>
                            </div>
                            <div className="p-6 border border-border/30 rounded-lg">
                                <Building2 className="h-6 w-6 text-slate-500 mb-4" />
                                <h3 className="font-semibold text-lg mb-2">Enterprise Controls</h3>
                                <p className="text-sm text-muted-foreground">
                                    SSO (SAML/OIDC), RBAC, dedicated support, SLAs, and custom data retention policies.
                                </p>
                            </div>
                        </div>
                    </div>
