...
export const H2 = ({ className, ...props }: React.ComponentProps<"h2">) => {
  ...
  return (
    <a
      href={`#${headingId}`}
      className="group text-primary relative no-underline"
    >
      <LinkIcon className="absolute top-1.25 -left-5 hidden size-4 translate-x-0.5 opacity-0 duration-200 ease-in-out group-hover:-translate-x-0.5 group-hover:opacity-100 lg:block" />
      <h2
        id={headingId}
        className={cn(
          "font-mono [&+]*:[code]:text-xl mt-10 scroll-m-10 text-xl font-medium tracking-tight select-none first:mt-0 [&+.steps]:mt-0! [&+.steps>h3]:mt-4! [&+h3]:mt-6! [&+p]:mt-4!",
          className,
        )}
        {...props}
      >
