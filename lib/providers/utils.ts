 ...
 * Returns true if providers are reachable, false if not
 */
export async function getProviderHealth(): Promise<boolean> {
  try {
    // Basic health check — returns true if the provider layer loads without error
    // Replace with real provider ping logic when available
    return true;
  } catch {
    return false;
  }
