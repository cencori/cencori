...
coverImage: "/newsroom/images/covers/build.png"
...
Today, we're simplifying this with **`create-cencori-app`**. This guide walks you through the 60-second path to a running AI application.
...
2.  **Include Demo Chat UI?**: 
    - Select **Yes** to inject a pre-built streaming chat component so you can test the gateway immediately.
...
Open the `.env.local` file (for Next.js) or `.env` (for TanStack). These files are ignored by git, and the scaffold also includes `.env.example` for sharing safe placeholders. You'll see a placeholder for your Cencori API key.
...
Now, start your development server. The CLI installs dependencies by default; if you used `--no-install`, run `npm install` first.
