...
| **Cencori Usage Charging** | Credit wallet is charged | No usage deduction; provider cost is still logged |
...
At a zero credit balance, requests that identify an active project BYOK key before execution can still proceed without a Cencori usage deduction. Ambiguous or multi-provider operations may require credits until their key source can be proven; a managed fallback will not run against an empty wallet.
