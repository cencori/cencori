---
title: MCP Server
description: Give MCP clients first-party web search, Cencori docs, Embedded Agents inspection, platform reads, inference, memory, sessions, and governance with explicit action tiers.
section: MCP
order: 1
---
...
`@cencori/mcp` is Cencori's official [Model Context Protocol](https://modelcontextprotocol.io) server. It gives Cursor, Claude Desktop, Codex, and any MCP client direct access to Cencori Web, documentation, Embedded Agents inspection, gateway metrics, memory, agents, sessions, governance, and multimodal inference.
...
Version `0.8.0` includes first-party Web tools and read-only Embedded Agents inspection.
...
| Read | `CENCORI_API_KEY` | Web search/fetch/extract, metrics, health, quota, Embedded Agents, and agents/memory/sessions/governance reads |
...
| `CENCORI_MCP_FEATURES` | all | Comma list: `docs`, `guidance`, `gateway`, `agents`, `memory`, `sessions`, `web`, `governance`, `multimodal`, `embedded`. |
...
- Embedded Agents: tenants, versions, installations, runs, actions, knowledge, skills, provider connections, usage, webhooks, and remote MCP snapshots

To expose only Embedded Agents inspection, set `CENCORI_MCP_FEATURES=embedded`. The feature is deliberately read-only; publishing, approvals, credentials, and other writes remain dashboard- or API-guided.
...
**No Embedded Agents tools appear with `CENCORI_MCP_FEATURES`:** include `embedded` and provide a secret project key.
