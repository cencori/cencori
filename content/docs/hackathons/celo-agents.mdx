...
- `DEMO_PAYMENT_*` fields are **metadata in the receipt JSON** only. The onchain step records the receipt hash via `recordRun` — it does not transfer USDC in this demo.
