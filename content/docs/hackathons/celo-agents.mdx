---
title: "Celo Agents Hackathon"
description: "Build a Cencori-powered agent with optional Celo Sepolia receipt proof. Quickstart for Celo hackathon builders."
section: "Hackathons"
order: 1
...
| Layer | What happens in the demo |
| --- | --- |
| **Cencori** | Real (or simulated) model call through the Gateway |
| **Receipt JSON** | Local file in `output/` with run metadata |
...
- A [Cencori account](https://cencori.com/signup).
...
1. In [Cencori](https://cencori.com/dashboard/organizations), open your project → **Agents** → create an agent.
...
- See [Payments vs onchain proof](#payments-vs-onchain-proof-read-this) — `DEMO_PAYMENT_*` does not move funds.
