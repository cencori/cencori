...
`POST /v1/agents/:id/runs` is idempotent (`Idempotency-Key` scoped to your project; same key + different body is `409`). Suspended tenants accept no new runs. Poll `GET /v1/runs/:id`, stream the durable log at `GET .../events?after=`, cancel with `POST .../cancel` — cancellation wins executor races via conditional claims, and cascades to active child runs.
