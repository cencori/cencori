...
Version `0.7.0` adds first-party Web tools.
...
All Web tools advertise `openWorldHint: true`. Their descriptions state that page content is untrusted data, never agent instructions. `web_browse` also rejects password, token, and secret field selectors.
