...
```bash
GET https://api.cencori.com/v1/metrics
```
...
```bash
curl -X GET "https://api.cencori.com/v1/metrics?period=24h" \
...
curl -X GET "https://api.cencori.com/v1/metrics?period=mtd" \
  -H "Authorization: Bearer sk_live_abc123"
```
...
async function getMetrics(period = '24h') {
  const response = await fetch(
    `https://api.cencori.com/v1/metrics?period=${period}`,
    {
      headers: {
...
def get_metrics(period='24h'):
    response = requests.get(
        f'https://api.cencori.com/v1/metrics?period={period}',
        headers={
