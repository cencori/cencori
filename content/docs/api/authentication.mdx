...
- **Project keys:** `csk_...` or `csk_test_...`
- **Publishable keys:** `cpk_...` or `cpk_test_...`
- **Agent-scoped keys:** `cake_...` for agent runtime flows
...
- **AI & Memory endpoints:** `CENCORI_API_KEY: csk_...` (preferred). `Authorization: Bearer csk_...` is also accepted.
- **OpenAI-compatible and REST endpoints (`https://api.cencori.com/v1/*`):** `Authorization: Bearer csk_...` is preferred because OpenAI-compatible SDKs expect it.
