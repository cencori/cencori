...
Arcie’s framework runner connects to the **Cencori Sessions API**. The `arcie/runner`
subpath gives you two entry points: `runAgent` (await a full result) and
`streamAgent` (consume events as they arrive).

The runner executes custom tools in its own process and sends results back to
continue the model turn. This framework interface is not the Cencori-managed
[Agent API](/arcie/docs/managed-api), which is in development.
...
`streamAgent` is an async generator that yields [protocol events](/arcie/docs/events)
as the turn unfolds:
...
- `{ "message": "…", "stream": true }` → newline-delimited [event](/arcie/docs/events) stream.
