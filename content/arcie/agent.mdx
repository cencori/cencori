...
| `description` | What this agent is for. **Required for [subagents](/arcie/docs/subagents)** so the orchestrator knows when to delegate. |
...
[Cencori integration](/arcie/docs/cencori) for the full picture.

<Callout type="info">
If `agent.ts` is missing or fails to load, Arcie falls back to a default config of
