...
spec.components.schemas = {
  ...
  RunOutput: object({
    type: string({ example: 'result' }),
    output: {},
    model: string(),
    provider: string(),
    agent_version_id: nullableString(),
    agent_version: nullableString(),
    knowledge_citations: array(record()),
    skills_used: array(string()),
    manifest_tools: array({}),
    manifest_policy: record(),
    usage: object({ promptTokens: integer(), completionTokens: integer(), totalTokens: integer(), cacheReadTokens: integer() }, ['promptTokens', 'completionTokens', 'totalTokens']),
    cost: object({ providerCostUsd: number(), cencoriChargeUsd: number(), markupPercentage: number() }, ['providerCostUsd', 'cencoriChargeUsd', 'markupPercentage']),
  }, ['type', 'output', 'model', 'provider', 'usage', 'cost']),
  Run: object({
    ...
    input: {}, output: { allOf: [ref('RunOutput')], nullable: true }, error: nullableString(), citations: array(record()),
    created_at: string({ format: 'date-time' }), updated_at: string({ format: 'date-time' }),
  }, ['id', 'agent_id', 'status']),
  ...
  CreateRunRequest: object({ installation_id: string(), tenant_id: string(), external_user_id: string(), mode: string({ enum: ['background', 'sync', 'streaming'] }), input: { description: 'Any JSON value, at most 64 KiB when serialized as UTF-8.' }, response_format: object({ type: string({ enum: ['json_schema'] }), json_schema: object({ name: string(), strict: boolean(), schema: record() }, ['schema']) }, ['type', 'json_schema']), session_id: string() }, [], { additionalProperties: false }),
